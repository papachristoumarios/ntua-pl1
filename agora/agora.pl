...
getNextStates(Parent, NextStates, NotVisited, Optimal) :-
		Parent = [Index, NNotVisited, Score, Holding],
		(
			NotVisited = [] -> !
		)

		head(NotVisited, Child),
		...
		(
			Res, New_score is Score + S,
						...
						write(Optimal), nl,
						head(Child, New_Index),
						sub(NotVisited, [Child], New_Not_Visited),
						QNext = [New_index, New_Not_Visited, New_score, New_Holding],
